...
//ver si marco lo retorna como lista y no como observable list
    ...
    public static List<Jugador>  listaPremioOpta() throws SQLException { // este metodo me devuelve una lista de jugadores con su nombre rannking su categoria y sobretod un array de la lista de los premiso a los que opta
        ...
        try{
            String sql = "SELECT fide, hotel, cv, categoria, nombre, rankingI FROM jugador ORDER BY categoria "; // saco las columnas que me interesan y las saco ordenados por categegoria (aunq creoq es indiferente)
            ps = cnx.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                int fide = rs.getInt("fide");
                ...
                Jugador jugador= new Jugador(rankingI, nombre, categoria); // creo el nuevo objeto jugador con los atributos que me interesan mostrar, no hace falta poner el array, ya esta inizializado en la clase
    ...
    public static List<Jugador> listaPremioGana() throws SQLException {
        List<Jugador> jugadores = listaPremioOpta();
        Map<String, Integer> prioridades = obtenerPrioridades();

        Set<String> premiosAsignados = new HashSet<>(); //Almacenar los premios ya asignados
        int posicion = 1; //Variable para seguir la posición del jugador

        for (Jugador jugador : jugadores) {
            List<Premio> premiosOpta = jugador.getPremios();
            premiosOpta.sort((p1, p2) -> {
                if (p1.getImporte() != p2.getImporte()) {
                    return Integer.compare(p2.getImporte(), p1.getImporte());
                } else {
                    return Integer.compare(prioridades.get(p1.getTipo()), prioridades.get(p2.getTipo()));
                }
            });

            for (Premio premio : premiosOpta) {
                if (!premiosAsignados.contains(premio.getTipo())) { // Verificar si el premio ya ha sido asignado
                    jugador.setPremioGanado(premio);
                    premiosAsignados.add(premio.getTipo());
                    jugador.setRankingF(posicion); // Establecer la posición del jugador
                    break; // Salir del bucle luego de asignar un premio
                }
            }
            posicion++; // Incrementar la posición para el siguiente jugador
        }
    ...
    //Obtenemos las prioridades para saber que premio gana cuando opta a varios
    private static Map<String, Integer> obtenerPrioridades() throws SQLException {
        Map<String, Integer> prioridades = new LinkedHashMap<>();
        try (PreparedStatement ps = cnx.prepareStatement("SELECT tipo, prioridad FROM premio");
             ResultSet rs = ps.executeQuery()) {

            while (rs.next()) {
                String tipo = rs.getString("tipo");
                int prioridad = rs.getInt("prioridad");
                prioridades.put(tipo, prioridad);
            }
        } catch (SQLException e) {
            throw new RuntimeException("Error al obtener las prioridades de los premios.", e);
        }
        return prioridades;
    }


    //Guardamos los jugadores con sus premios ganados en un archivo
    public static void guardarPremiosGanadosArchivo(List<Jugador> jugadores, String nombreArchivo) throws IOException {
        try (FileWriter fw = new FileWriter(nombreArchivo)) {
            fw.write("Premios ganados por los jugadores\n\n");
            for (Jugador jugador : jugadores) {
                fw.write(jugador.getRankingI() + " " + "Torneo: " + jugador.getTipoTorneo() + " - " + jugador.getNombre() + "\n");
                Premio premioGanado = jugador.getPremioGanado();
                if (premioGanado != null) {
                    fw.write("\tPremio ganado: " + premioGanado.getTipo() + "\n");
                } else {
                    fw.write("\tNo ganó ningún premio\n");
                }
                fw.write("\n");
            }
        } catch (IOException e) {
            throw new RuntimeException("Error al guardar en el archivo.", e);
        }
    }
}
